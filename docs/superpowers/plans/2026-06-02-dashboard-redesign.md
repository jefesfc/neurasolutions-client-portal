...
**Goal:** Replace WelcomeBanner + KPICardGrid with a single rich HeroBanner component showing real data, four chart panels, and an Active Services card — in the approved Design B v6 dark/gold color scheme.

**Architecture:** One new `HeroBanner` component fetches all data (leads, contacts, token_usage) and renders the full hero + bottom KPI row. Static panels (Platform Health, Security Health, Active Services) are inlined rather than split into sub-files to avoid unnecessary abstraction.

**Tech Stack:** React 18, TypeScript, Tailwind v4 inline styles (dark theme), `useQuery<T>` hook against PostgREST, SVG donut charts (no external chart lib needed for this design), Lucide React icons, react-router-dom Link.
...
## Task 1: Create HeroBanner — data layer + skeleton

**Files:**
- Create: `src/components/dashboard/HeroBanner.tsx`

- [ ] **Step 1: Create the file with data fetching and computed values**
...
Run: `cd AIOS && npx tsc --noEmit`  
Expected: exits 0 (or only pre-existing errors, none from HeroBanner.tsx)

---

## Task 2: Build the hero wrapper + left panel

**Files:**
- Modify: `src/components/dashboard/HeroBanner.tsx`

- [ ] **Step 1: Replace the `return` block with the full hero layout**

Replace the `return (...)` in `HeroBanner.tsx` with:
      ...
      <div style={{
        ...
        <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
            {[
              { label: "Total Leads",     value: leads.length,   sub: "↑ active pipeline" },
              { label: "Active Contacts", value: activeContacts,  sub: "↑ this month" },
...
Run: `cd AIOS && npx tsc --noEmit`  
Expected: may show "cannot find name X" for sub-components not yet defined — that's fine. No structural errors.

---

## Task 3: Add ActiveServicesCard sub-component (same file)

**Files:**
- Modify: `src/components/dashboard/HeroBanner.tsx` — add before `HeroBanner` function

- [ ] **Step 1: Insert ActiveServicesCard function above the HeroBanner export**
...
- [ ] **Step 2: Add keyframe CSS for pulse animation**

Add a `<style>` tag render or a global CSS approach. Since AIOS uses Tailwind, add the keyframe to `src/index.css` (or main CSS file):
<br>First check what the main CSS file is:

Run: `ls /c/Users/ldmru/OneDrive/Desktop/Neura/AIOS/src/*.css`

Then add to that file (likely `src/index.css`):
...
Run: `cd AIOS && npx tsc --noEmit`  
Expected: 0 new errors from HeroBanner.tsx

---

## Task 4: Add LeadsStatusChart sub-component (same file)

**Files:**
- Modify: `src/components/dashboard/HeroBanner.tsx` — add before `HeroBanner`

- [ ] **Step 1: Insert LeadsStatusChart function**
...
- [ ] **Step 2: Add shared style constants above all sub-components**

Add these shared style objects at the top of the file (after imports):
...
const chartTitleStyle: React.CSSProperties = {
  ...
  borderBottom: "1px solid rgba(255,215,0,0.1)", paddingBottom: 8,
};
```

Also add `import React from "react";` at the top if not already present (needed for `React.CSSProperties`).
...
## Task 5: Add PlatformHealthPanel sub-component

**Files:**
- Modify: `src/components/dashboard/HeroBanner.tsx`

- [ ] **Step 1: Insert PlatformHealthPanel function**
...
## Task 6: Add AICostPanel sub-component

**Files:**
- Modify: `src/components/dashboard/HeroBanner.tsx`

- [ ] **Step 1: Insert AICostPanel interface + function**
...
## Task 7: Add SecurityHealthPanel sub-component

**Files:**
- Modify: `src/components/dashboard/HeroBanner.tsx`

- [ ] **Step 1: Insert SecurityHealthPanel function (static placeholder)**
...
## Task 8: Add BottomKpiRow sub-component

**Files:**
- Modify: `src/components/dashboard/HeroBanner.tsx`

- [ ] **Step 1: Insert BottomKpiRow function**
...
Run: `cd AIOS && npx tsc --noEmit`  
Expected: 0 new errors — HeroBanner.tsx fully typed

---

## Task 9: Wire up DashboardPage + remove old components
...
- [ ] **Step 2: Delete WelcomeBanner.tsx**

Run: `rm /c/Users/ldmru/OneDrive/Desktop/Neura/AIOS/src/components/dashboard/WelcomeBanner.tsx`

- [ ] **Step 3: Delete KPICardGrid.tsx**

Run: `rm /c/Users/ldmru/OneDrive/Desktop/Neura/AIOS/src/components/dashboard/KPICardGrid.tsx`
...
```bash
git add src/components/dashboard/HeroBanner.tsx \
        src/pages/DashboardPage.tsx \
        src/index.css \
        docs/superpowers/specs/2026-06-02-dashboard-redesign.md \
        docs/superpowers/plans/2026-06-02-dashboard-redesign.md
git rm src/components/dashboard/WelcomeBanner.tsx \
       src/components/dashboard/KPICardGrid.tsx
git commit -m "feat(dashboard): replace WelcomeBanner+KPICardGrid with HeroBanner (Design B v6)"
```
...
Run: `cd /c/Users/ldmru/OneDrive/Desktop/Neura/AIOS && npm run dev`

- [ ] **Step 2: Open dashboard**

Navigate to `http://localhost:5173` (or whichever port Vite uses) → login → Dashboard.

- [ ] **Step 3: Verify layout**

Check:
- [ ] Hero renders: left panel + 2×2 right grid + bottom row all visible
- [ ] Left stats show real numbers (not 0/hardcoded) — Total Leads, Active Contacts, Conversion, AI Cost
- [ ] Leads by Status donut matches actual leads in DB
- [ ] AI Cost Breakdown shows real agents from token_usage (or "No token usage" if table is empty)
- [ ] Active Services card shows 6 rows with correct status pills
- [ ] Bottom KPI row: 4 dark cards with gold values
- [ ] No console errors / TypeScript errors

- [ ] **Step 4: Check responsive on smaller viewport**

Resize browser to ~1200px wide. Hero should still be legible (left/right columns may overlap — acceptable for initial implementation; responsive tweaks are out of scope).
