    ...
    <PageTransition>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-400">No contacts found</div>
        ) : (
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((contact) => (
                <tr key={contact.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-slate-500">{contact.company ?? "—"}</td>
                  ...
                  <td className="px-4 py-3 text-slate-400">{formatRelative(contact.created_at)}</td>
                </tr>
              ))}
            </tbody>
          </table>
